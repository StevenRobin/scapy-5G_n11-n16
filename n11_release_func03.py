...
def process_multipart_content(content, cgi1, tac1, imsi1, pkt_idx):
    ...
    try:
        ...
        for i, part_bytes in enumerate(parts_bytes):
            ...
            if is_binary_part:
                # 二进制部分：完全不处理，保持原始字节
                print(f"报文{pkt_idx}: 严格保护二进制部分 {i}（保持原始字节）")
                modified_parts_bytes.append(part_bytes)
                continue
                    ...
                    if headers_end > 0:
                        ...
                        if json_start != -1 and json_end > json_start:
                            ...
                            if modified:
                                ...
                                print(f"报文{pkt_idx}: JSON部分修改成功")
                            else:
                                modified_parts_bytes.append(part_bytes)
                                print(f"报文{pkt_idx}: JSON部分无需修改")
                        else:
                            modified_parts_bytes.append(part_bytes)
                            print(f"报文{pkt_idx}: 未找到有效JSON")
                    else:
                        # 无法分离headers，保持原样
                        modified_parts_bytes.append(part_bytes)
                
                except (json.JSONDecodeError, Exception) as e:
                    print(f"报文{pkt_idx}: JSON处理失败 {e}，保持原样")
                    modified_parts_bytes.append(part_bytes)
            else:
                # 其他部分保持不变（字节模式）
                modified_parts_bytes.append(part_bytes)
        ...
        if len(result_bytes) < len(content_bytes) * 0.5:
            print(f"报文{pkt_idx}: 警告 - 处理后内容异常小，使用原始内容")
            return content if isinstance(content, str) else content.decode('utf-8', errors='replace')
        
        # 验证multipart结构
        if result_bytes.count(boundary_bytes) < 2:
            print(f"报文{pkt_idx}: 警告 - multipart结构可能损坏，使用原始内容")
            return content if isinstance(content, str) else content.decode('utf-8', errors='replace')
        
        print(f"报文{pkt_idx}: multipart处理完成，长度: {len(content_bytes)} -> {len(result_bytes)}")
        ...
        print(f"报文{pkt_idx}: multipart处理异常: {e}")
        # 任何异常都返回原始内容
        return content if isinstance(content, str) else content.decode('utf-8', errors='replace')
