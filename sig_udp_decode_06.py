...
def format_china_time(val):
    if val is None:
        return ""
    seconds = val // 1_000_000
    useconds = val % 1_000_000
    t_utc = datetime.datetime.utcfromtimestamp(seconds)
    t_china = t_utc + datetime.timedelta(hours=8)
    t_str = t_china.strftime("%Y-%m-%d %H:%M:%S")
    return f"{t_str}.{useconds:06d} 中国时间"
...
def parse_main_fields(payload):
    ...
    for name, offset, size, fmt, hexw in MAIN_FIELDS:
        ...
        if fmt.endswith('s'):
            ...
            res[name] = format_china_time(val)
        else:
            val = struct.unpack_from(fmt, payload, offset)[0]
...
def print_separator(packet_num, width=60):
    """打印带有报文序号的分隔符"""
    packet_text = f"pkt{packet_num}"
    left_width = (width - len(packet_text)) // 2
    right_width = width - left_width - len(packet_text)
    print("-" * left_width + packet_text + "-" * right_width)
