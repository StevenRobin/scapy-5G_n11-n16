...
此脚本是对n16_test_fixed02.py的补充，直接使用硬编码的HPACK头部进行修复
"""
...
def fix_packet_15():
    """直接修复第15个数据包的所有问题"""
    # 配置文件路径
    input_file = "pcap/N16_1516.pcap"  # 使用主脚本生成的PCAP
    ...
    if data_frame is None:
        logger.warning("未找到有效的DATA帧，使用硬编码的DATA帧")        data_frame = bytes.fromhex(
            "00000159" +  # 长度 (345字节)
            "00" +        # 类型 (DATA帧)
            "00" +        # 标志
            "00000001" +  # 流ID
