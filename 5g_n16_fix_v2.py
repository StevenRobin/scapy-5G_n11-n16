        ...
        for name, value in headers:
            ...
            if name.lower() == 'content-length':
                if not content_length_found:
                    ...
                    print(f"[+] 设置 content-length: {value} -> {content_length}")
    ...
    try:
        ...
        for name, value in headers:
            ...
            if name.lower() == "content-length":
                ...
                # 跳过，稍后会添加正确的content-length
                continue
                
            if name == ":path":
                if "sm-contexts" in value and "retrieve" in value:
        ...
        # 如果存在原始content-length并且提供了准确的DATA长度，添加修正后的content-length
        if original_content_length is not None and exact_data_length is not None:
            print(f"[+] 调整 content-length: {original_content_length} -> {exact_data_length}")
            new_headers.append(('content-length', str(exact_data_length)))
            modified = True
        # 如果不存在原始content-length但提供了DATA长度，添加新的content-length
        elif original_content_length is None and exact_data_length is not None:
            print(f"[+] 添加 content-length: {exact_data_length}")
            new_headers.append(('content-length', str(exact_data_length)))
            modified = True
        # 如果存在原始content-length但没有提供DATA长度，保留原始content-length
        elif original_content_length is not None:
            print(f"[+] 保留 content-length: {original_content_length}")
            new_headers.append(('content-length', original_content_length))
...
def process_specific_packet(raw, context_num, modifications):
    ...
    try:
        ...
        if data_frames:
            ...
            modified_data, _ = process_http2_data_frame(data_frame['data'], modifications)
            if modified_data:
            ...
            print(f"[处理] DATA帧长度: {data_length}")
        ...
        for frame in frames:
            if frame['type'] == 0x1:  # HEADERS帧
                ...
                if 'content-length' in headers and data_length is not None:
                    # 确保content-length与DATA帧长度一致
                    modified_frame_data = set_headers_content_length(frame['data'], data_length)
                    frame_header = frame['header']
                    frame_header.length = len(modified_frame_data)
                    new_payload += bytes(frame_header.build()) + modified_frame_data
                else:
                    # 普通HEADERS帧处理
                    modified_frame_data = process_http2_headers_frame(frame['data'], context_num)
                    if modified_frame_data != frame['data']:
                        frame_header = frame['header']
                        frame_header.length = len(modified_frame_data)
                        new_payload += bytes(frame_header.build()) + modified_frame_data
                    else:
...
def check_content_length_mismatch(raw):
    ...
    try:
        ...
        if content_length is not None and data_length is not None:
            return content_length != data_length, data_length
        return False, None
    except:
        return False, None
    ...
    if pkt.haslayer(TCP):
        ...
        if has_payload and not (is_syn or is_fin or is_rst):
            ...
            if is_http2_packet(raw):
                ...
                has_mismatch, actual_length = check_content_length_mismatch(raw)
                
                if has_mismatch:
                    print(f"[检测] 发现content-length不匹配报文: {pkt[IP].src}:{pkt[TCP].sport}->{pkt[IP].dst}:{pkt[TCP].dport}")
                    # 对特定报文（第11、13、15号等）进行专门处理
                    new_payload = process_specific_packet(raw, context_num, modifications)
                else:
                    # 常规HTTP/2报文处理，但仍然确保content-length与DATA帧长度一致
                    new_payload = process_specific_packet(raw, context_num, modifications)
            else:
                # 非HTTP/2报文，保持原样
                new_payload = raw
...
def main():
    ...
    PCAP_OUT = "pcap/N16_166.pcap"   # 输出 PCAP 文件路径
