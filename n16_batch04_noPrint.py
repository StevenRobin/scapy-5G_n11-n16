...
def main():
    ...
    try:
        ...
        loop_iter = tqdm(range(LOOP_NUM), desc="Batch Loop")
    except ImportError:
        loop_iter = range(LOOP_NUM)

    for i in loop_iter:
        # logger.info(f"==== 处理第{i+1}组报文 ====")
        update_global_vars(i)
        seq_diff = {}
        modified_packets = []
        try:
            pkt_iter = tqdm(enumerate(orig_packets, 1), total=len(orig_packets), desc=f"Packets {i+1}", leave=False)
        except Exception:
            pkt_iter = enumerate(orig_packets, 1)
        for idx, pkt in pkt_iter:
            pkt = pkt.copy()
            modified = False
        ...
        all_modified_packets.extend(modified_packets)

    # logger.info(f"保存批量修改后的PCAP到 {PCAP_OUT}")
    wrpcap(PCAP_OUT, all_modified_packets)
    # logger.info(f"批量处理完成，共处理 {len(all_modified_packets)} 个报文")
