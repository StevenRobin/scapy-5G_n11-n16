    ...
    try:
        ...
        for name, value in headers:
            if name == ":path":
                ...
                modified = True
            elif name != "content-length":  # 暂时跳过content-length
                new_headers.append((name, value))
        
        # 最后处理content-length，确保它总是反映实际数据长度
        if data_length is not None:
            print(f"[+] 设置 header content-length: {data_length}")
...
def process_packet(pkt, seq_diff, ip_replacements, modifications):
    stream_data_lengths = {}  # 记录各流DATA帧长度
            ...
            while temp_offset < len(raw):
                ...
                if ft == 0x0:  # DATA帧
                    ...
                    if modified_data:
                        ...
                        print(f"[+] 流 {fh.stream_id} 的DATA帧长度更新为: {len(modified_data)}")
                temp_offset = fe
...
PCAP_OUT = "pcap/N16_modified_content_length.pcap"
