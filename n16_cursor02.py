...
def process_http2_headers_frame(frame_data, context_num, data_length=None):
    ...
    try:
        ...
        if data_length is not None and data_length > 0:
            ...
            content_length_value = str(data_length)  # 更新为新值
            modified = True

        # 返回原始content-length值用于调试输出
        if modified:
        ...
        if has_payload and not (is_syn or is_fin or is_rst):
            ...
            while current_offset < len(raw):
                ...
                if frame_type == 0x1:  # HEADERS帧
                    ...
                    actual_data_length = frame_len  # 记录DATA帧的实际长度
                current_offset = frame_end
            ...
            while offset < len(raw):
                ...
                if frame_type == 0x1:  # HEADERS帧
                    ...
                    modified_frame_data, cl_value = process_http2_headers_frame(frame_data, context_num, current_json_length)
                    if cl_value:
                        content_length_value = cl_value
                    if isinstance(modified_frame_data, bytes):
                    ...
                    if modified_frame_data:
                        ...
                        frame_header.length = frame_len
                        new_payload += frame_header.build() + modified_frame_data
                        actual_data_length = frame_len  # 更新实际DATA帧长度
                        offset = frame_end
                        continue
            ...
            # 打印调试信息：content-length 和 DATA帧实际长度
            if content_length_value is not None or actual_data_length is not None:
                print(f"[调试] 报文({pkt[IP].src}:{pkt[TCP].sport} -> {pkt[IP].dst}:{pkt[TCP].dport}):")
                if content_length_value is not None:
                    print(f"    Content-Length值: {content_length_value}")
                if actual_data_length is not None:
                    print(f"    DATA帧实际字节长度: {actual_data_length}")
                if content_length_value and actual_data_length and int(content_length_value) != actual_data_length:
                    print(f"    [警告] Content-Length({content_length_value})与DATA帧长度({actual_data_length})不匹配!")
...
PCAP_OUT = "pcap/N16_1601.pcap"   # 输出 PCAP 文件路径
