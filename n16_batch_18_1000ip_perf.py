...
def update_global_vars(i, ip_num=1000, tac_num=1000000):
...
def process_one_group(i, orig_packets_bytes, ip_num=1000, tac_num=1000000):
    # 反序列化
    orig_packets = rdpcap(orig_packets_bytes)
    update_global_vars(i, ip_num, tac_num)
    seq_diff = {}
    modified_packets = []
    ...
    with ThreadPoolExecutor(max_workers=4) as file_writer:
        with concurrent.futures.ProcessPoolExecutor(max_workers=6) as executor:
            for i in range(total_batches):
                ...
                func = partial(process_one_group, orig_packets_bytes=orig_packets_bytes, ip_num=IP_NUM, tac_num=TAC_NUM)
