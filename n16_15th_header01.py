...
def process_special_headers(frame_data, pkt_idx):
    """特殊处理HTTP2 Headers帧 - 确保第15号报文的头部字段保持正确"""
    try:
        ...
        if pkt_idx == 15:
            ...
            try:
                # 直接创建预定义的header块 - 适用于第15号报文
                # 按照Wireshark捕获的正确顺序设置字段
                fixed_headers = [
                    (b':status', b'201'),  # status必须为201，不是201 Created
                    (b'content-type', b'application/json'),
                    (b'location', f"http://{auth1}/nsmf-pdusession/v1/pdu-sessions/{context_ID}".encode()),
                    (b'date', b'Wed, 22 May 2025 02:48:05 GMT'),
                    (b'content-length', b'351')  # 由外部函数稍后会更新为正确值
                ]
    ...
    try:
        ...
        if current_pkt_idx == 15:
            try:
                ...
                # 对于第15号报文，如果HPACK解析失败，尝试二进制替换
                try:
                    # 二进制方式替换content-length
...
def main():
    ...
    parser.add_argument('-i', '--input', dest='input_file', default="pcap/N16_create_16p.pcap",
                        help='输入PCAP文件路径')
    parser.add_argument('-o', '--output', dest='output_file', default="pcap/N16_1704.pcap",
                        help='输出PCAP文件路径')
