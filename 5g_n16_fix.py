...
def handle_pdu_session_frames(raw, context_num, modifications):
    ...
    try:
        ...
        # 第一遍：扫描并找到数据帧
        data_frame_offset = None
        headers_with_cl_offset = None
        current_offset = 0
        ...
        while current_offset + 9 <= len(raw):
            ...
            if frame_type == 0x0:  # DATA帧
                data_frame_offset = current_offset
                modified_data, data_frame_length = process_http2_data_frame(frame_data, modifications)
            elif frame_type == 0x1 and b"content-length" in frame_data.lower():  # 包含content-length的HEADERS帧
                headers_with_cl_offset = current_offset
        ...
        while offset + 9 <= len(raw):
            ...
            if offset == headers_with_cl_offset:  # 处理包含content-length的HEADERS帧
                # 确保我们先处理了DATA帧并获取了准确的长度
                if data_frame_length:
                    processed_headers, _ = process_http2_headers_frame(frame_data, context_num, data_frame_length)
                    frame_len = len(processed_headers)
                    frame_header.length = frame_len
                    new_payload += bytes(frame_header.build()) + processed_headers
                else:
                    new_payload += raw[offset:offset + 9 + frame_len]
            elif offset == data_frame_offset and modified_data:  # 处理DATA帧
                frame_header.length = len(modified_data)
                new_payload += bytes(frame_header.build()) + modified_data
            else:  # 处理其他帧
                if frame_type == 0x1:  # 其他HEADERS帧
                    processed_headers, _ = process_http2_headers_frame(frame_data, context_num, None)
                    if processed_headers != frame_data:
                        frame_len = len(processed_headers)
                        frame_header.length = frame_len
                        new_payload += bytes(frame_header.build()) + processed_headers
                        offset += 9 + frame_len
        ...
        print(f"[特殊处理] PDU会话创建请求处理完成，DATA帧长度: {data_frame_length or '未知'}")
        return new_payload
    except Exception as e:
...
def main():
    ...
    PCAP_OUT = "pcap/N16_163.pcap"   # 输出 PCAP 文件路径
